...
module.exports = class GitHubActionTemplate extends require('../../plugin') {
  ...
  async run() {
    const { hasMta, hasHelm, isJava } = readProject()
    ...
    if (!exists(release) || cds.cli.options.force) {
      ...
      await renderAndCopy(path.join(__dirname, 'files/workflows/release.yaml.hbs'), release, { isJava, sap: cds.cli.options.sap, githubSlug, hasMta, hasHelm })
    } else {
