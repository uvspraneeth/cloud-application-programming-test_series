...
function info({ root: cwd = process.cwd() }) {
  ...
  for (let k of ['@sap/cds']) if (!(k in sorted)) sorted[k] = versions[k]
...
function _markdown (versions, { root: cwd = process.cwd() }) {
  const pkg = { name:'', repository:'', version:'' }; try {
    Object.assign (pkg, require (require.resolve('./package.json', { paths:[cwd] })))
  } catch (e) {/* ignored */}
